...
while [ "${YMDH}" -gt "${YMDH_CUT}" ]; do
    ...
    # Create the full ECMWF directory
    EC_DIR_FULL="${EC_DIR}/${YMD}/${HH}z/${EC_MODEL}/${EC_APP}/"

    # Download the data with wget commands
    wget -r -A "*.grib2" -T 5 -N ${EC_DIR_FULL} -P ${ODIR_FULL}
    wget -r -A "*.index" -T 5 -N ${EC_DIR_FULL} -P ${ODIR_FULL}
    wget -r -A "*.bufr" -T 5 -N ${EC_DIR_FULL} -P ${ODIR_FULL}
