        ...
        for CYCLE in ${ALL_CYCLES[@]}; do
            ...
            for BASIN in ${ALL_BASINS[@]}; do
                ...
                if [ "${BASIN,,}" = "al" ]; then
                    ...
                    BASIN2="w"
                    echo "MSG: ATCF basin --> western North Pacific"
                elif [ "${BASIN,,}" = "io" ]; then
                    BASIN2="b"
                    #BASIN2="a"
                    ...
                    BASIN2="s"
                    echo "MSG: ATCF basin --> South Hemisphere"
                elif [ "${BASIN,,}" = "ls" ]; then
                    BASIN2="q"
                    echo "MSG: ATCF basin --> South Atlantic"
                else
                ...
                # Initialize the TC Name as empty. This is for the Parsed ATCF file name.
                TCNAME=""
    
                # First, try to get the TC name from the Best Track entry in the B-Deck
                if [ -z "${TCNAME}" ] && [ ! -z "${BDECK}" ]; then
                    #TCNAME="`awk -v CYCLE="${CYCLE}" '$3~CYCLE' ${BDECK} | head -1 | cut -d "," -f28 | awk '{$1=$1};1'`"
                    TCNAME="`awk -v CYCLE="${CYCLE}" -v BASIN="${BASIN^^}" -v SNUM="${SNUM}" -F, '$1==BASIN && $2==SNUM && $3==CYCLE' ${BDECK} | head -1 | cut -d "," -f28 | awk '{$1=$1};1'`"
                fi
    
                # Second, try to get the TC name from the TCVItals entry
                if [ -z "${TCNAME}" ] && [ ! -z "${SYNDAT}" ]; then
                ...
                if [ -z "${TCNAME}" ]; then
                    TCNAME="`cat ${ADECK} | tr -d "[:blank:]" | awk -v BASIN="${BASIN^^}" -v CYCLE="${CYCLE}" -v SNUM="${SNUM}" -F, '$1==BASIN && $2==SNUM && $3==CYCLE && $5=="CARQ"' | head -1 | cut -d "," -f28`"
                fi
                ...
                # If TCNAME is "INVEST", but Storm Number < 50, try to rename it.
                ...
                #OFILE2="${OUTDIR}/${TCNAME,,}${SID_OLD,,}.${CYCLE}.${OTAG}"
                #OFILE3="${OUTDIR}/invest${SID_OLD,,}.${CYCLE}.${OTAG}"
                #if [ -z "${SID_OLD}" ]; then
                ...
                if [ -f ${OFILE} ]; then
                    ...
                    cat ${ADECK} | awk -v BASIN="${BASIN^^}" -v SNUM="${SNUM}" -v CYCLE="${CYCLE}" -v MODEL="${MODEL^^}" -F ',[ \t]*' \
                                   '$1==BASIN && $2==SNUM && $3==CYCLE && $5==MODEL' > ${TMPFILE}
                    if [ ! -z "${OMODEL}" ]; then
                        sed -i 's/'"${MODEL}"'/'"${OMODEL}"'/g' ${TMPFILE}
                    fi
                    cat ${OFILE} ${TMPFILE} | tac | sort -s -t, -k3,3 -k5,5 -k6,6n -k12,12 -u > ${TMPFILE2}
                    rm -f ${TMPFILE}
                    DIFF=$(diff ${TMPFILE2} ${OFILE})
                    if [ "${DIFF}" != "" ]; then
                        echo "MSG: Moving new deck to target location"
                        echo "MSG: ${TMPFILE2} --> ${OFILE}"
                        ...
                        echo "MSG: Removing this temporary file --> ${TMPFILE2}"
                        rm -f ${TMPFILE2}
                    fi
