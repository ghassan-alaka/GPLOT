...
# print STDERR "file=$tfile t_index=$t_index\n";
...
#  make inventory of times 1, 2 and last
   system "$wgrib2 $wflag $tflag -lev -lev0 -ctl_ens -ftime -ctl_inv \"$dir$file1\" >$ListA";
   if ($file1 ne $file2) {
      system "$wgrib2 $wflag $tflag -lev -lev0 -ctl_ens -ftime -ctl_inv \"$dir$file2\" >>$ListA";
   }
   if ($file2 ne $filelast) {
      system "$wgrib2 $wflag $tflag -lev -lev0 -ctl_ens -ftime -ctl_inv \"$dir$filelast\" >>$ListA";
   }
}
else {
   print  STDERR "wgrib2 4\n";
