...
echo "MSG: spawn_ocean_obs.sh started at `date`"
echo "MSG: Submitting jobs for GPLOT module 'OCEAN_OBS'."
...
DO_OCEAN_OBS="`sed -n -e 's/^DO_OCEAN_OBS =\s//p' ${NMLIST} | sed 's/^\t*//'`"
...
OCEAN_OBS_PYTHONFILE="`sed -n -e 's/^OCEAN_OBS_PYTHONFILE =\s//p' ${NMLIST} | sed 's/^\t*//'`"
FIX_DIR="`sed -n -e 's/^FIX_DIR =\s//p' ${NMLIST} | sed 's/^\t*//'`"
OCEAN_WRAP_LON="`sed -n -e 's/^OCEAN_WRAP_LON =\s//p' ${NMLIST} | sed 's/^\t*//'`"
OCEAN_OBS_DIR="`sed -n -e 's/^OCEAN_OBS_DIR =\s//p' ${NMLIST} | sed 's/^\t*//'`"
...
if [ -z "${OCEAN_OBS_DIR}" ]; then
    case ${MACHINE^^} in
        "JET")
            OCEAN_OBS_DIR="/mnt/lfs4/HFIP/aoml-hafs1/Lew.Gramer/ocean/data"
            ;;
        "HERA")
            OCEAN_OBS_DIR="/scratch2/AOML/aoml-hafs1/Lew.Gramer/ocean/data"
            ;;
        "ORION")
            OCEAN_OBS_DIR="/work2/noaa/aoml-hafs1/lgramer/ocean/data"
            ;;
        *)
            OCEAN_OBS_DIR="${GPLOT_DIR}/ocean_obs/"
            ;;
    esac
fi
...
OCEAN_OBS_DOMAIN=( `sed -n -e 's/^OCEAN_OBS_DOMAIN =\s//p' ${NMLIST} | sed 's/^\t*//'` )
echo "MSG: Found these graphic Ocean OBS domains in the namelist --> ${OCEAN_OBS_DOMAIN[*]}"

# Get the graphics tiers for this OCEAN_OBS domain
...
####################################################################
# 2. CALL GPLOT OCEAN OBS                                          #
#    This script is responsible for creating 2D plan view graphics #
#    for storm-centered domains, comparing ocean model with obs.   #
####################################################################
if [ "${DO_OCEAN_OBS}" = "True" ]; then
    OCEAN_OBS_PYTHONFILE="${OCEAN_OBS_PYTHONFILE:-plot_ocean_obs.py}"
    BATCHFILE="batch_ocean_obs.sh"
    #DOMAIN="ocean"
    TIER=( "Tier1" )
    SC="True"
    ATCF_REQD="True"
    ...
    for TR in ${TIER[@]}; do
        ...
        for CYCLE in ${CYCLES[@]}; do
            ...
            if [ "${IS_ENS}" == "True" ]; then
                :
            elif [ "${IS_MSTORM}" == "True" ] && [ "${STORMS[*]}" != "NONE" ]; then
                STORMS+=("00L")
            fi
            ...
            for STORM in ${STORMS[@]}; do
                ...
                for DMN in ${OCEAN_OBS_DOMAIN[@]}; do
                    ...
                    for ID in ${ENSIDS[@]}; do
                        ...
                        if [ "${ODIR_TYPE}" == "1" ]; then
                            ODIR_FULL="${ODIR}/${ENSID_DIR}/ocean_${DMN}_obs/"
                        else
                            ODIR_FULL="${ODIR}/${EXPT}/${CYCLE}/${ENSID_DIR}/ocean_${DMN}_obs/"
                        fi
                        ...
                        while [ -z "${IFILES[*]}" ]; do
                            ...
                            for FHR in ${FILE_FHRS[@]}; do
                                ...
                                #DEBUG:                                echo "DEBUG:: #FILE_LS@: ${#FILE_LS[@]}"
                                #DEBUG:                                echo "DEBUG:: FILE_LS_0: ${FILE_LS[0]}" ; echo "DEBUG:: FILE_LS_1: ${FILE_LS[1]}" ; echo "DEBUG:: FILE_LS_2: ${FILE_LS[2]}"
                                if [ "${#FILE_LS[@]}" -eq "1" ]; then
                                    IFILES+=("${FILE_LS[*]}")
                                    IFHRS+=( ${FHR} )
                                else
                                    FILE_LS=( `ls ${FILE_SEARCH2} 2>/dev/null` )
                                    #DEBUG:                                    echo "DEBUG:: FILE_SEARCH2: ${FILE_LS}"
                                    #DEBUG:                                    echo "DEBUG:: #FILE_LS@: ${#FILE_LS[@]}"
                                    if [ "${#FILE_LS[@]}" -eq "1" ]; then
                                        IFILES+=("${FILE_LS[*]}")
                                        IFHRS+=( ${FHR} )
                        ...
                        if [ -z "${IFILES[*]}" ]; then
                            ...
                            echo "WARNING: Skipping ocean_obs for this case; check that ocean output exists for this cycle."
                        ...
                        PLOTTED_FILE="${ODIR_FULL}PlottedOceanFiles.${DMN}.${TR}${STORMTAG}.log"
                        ...
                        if [ "${BATCH_MODE^^}" == "FOREGROUND" ]; then
                            ...
                            JOB_NAME="GPLOT.${EXPT}.${CYCLE}${ENSIDTAG}.ocean_${DMN}_obs${STORMTAG}.${TR}"
                        ...
                        if [ -z "${JOB_TEST}" ]; then
                            ...
                            # Change options in the batch submission script.
                            LOG_DIR="${ODIR_FULL}"
                            LOGFILE1="${LOG_DIR}GPLOT_Ocean_Obs.${EXPT}.${CYCLE}${ENSIDTAG}.${DMN}${STORMTAG}.${TR}.log"
                            LOGFILE2="${LOG_DIR}GPLOT_Ocean_Obs.${EXPT}.${CYCLE}${ENSIDTAG}.${DMN}${STORMTAG}.${TR}.out"

                            # Submit the child batch job.
                            echo "MSG: Submitting GPLOT child batch job. BATCH_MODE ${BATCH_MODE}, DMN ${DMN}"
                            FULL_CMD="${BATCH_DIR}/${BATCHFILE} ${MACHINE} ${PYTHON_DIR}${OCEAN_OBS_PYTHONFILE} ${LOGFILE1} ${NMLIST} ${ENSID}"
                            FULL_CMD="${FULL_CMD} ${CYCLE} ${STORM} ${DMN} ${TR} ${RESOLUTION} ${RMAX} ${LEVS} ${FORCE}"
                            FULL_CMD="${FULL_CMD} ${OCEAN_SOURCE} ${OCEAN_CFG} ${FIX_DIR} ${OCEAN_WRAP_LON} ${OCEAN_OBS_DIR}"
                            if [ "${BATCH_MODE^^}" == "FOREGROUND" ]; then
                            ...
                            if [ "$N" -ge "$MAX_JOBS" ]; then
                                ...
                                echo "MSG: spawn_ocean_obs.sh completed at `date`"
                                exit
                            fi
                        else
