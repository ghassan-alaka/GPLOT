...
def load_nge_data(BASEFDT,ndays=(-2,-1,0,1,2,3,4),ddir=default_ddir):
  ...
  ds['CalcMLT'] = ds.CT.where(ds.CT.pressure>ds.MLD).mean();
  ds['CalcSSS'] = ds.SA[:,0];
  ds['CalcMLS'] = ds.SA.where(ds.CT.pressure>ds.MLD).mean();
  return(ds);
...
def load_argo_data(BASEFDT,ndays=(-3,-2,-1,0,1,2,3,4),ddir=default_ddir):
  ...
  for nday in ndays:
    ...
    argo = argo.dropna('N_PROF','all',subset=['TEMP']);
...
def find_domain_argos(argos,corners,timerng=None):
  ...
  for argo in argos:
    if ( timerng is not None ):
      idxs = np.where( (corners[0]<=argo.LONGITUDE) & (argo.LONGITUDE<=corners[2]) & (corners[1]<=argo.LATITUDE) & (argo.LATITUDE<=corners[3]) & (timerng[0]<=argo.JULD) & (argo.JULD<=timerng[1]) )
    else:
...
def plot_profile(axT,axO,axP,lon,lat,z,T,OHC=None,mdlds=None,color='k',marker='d',linewidth=1.5,var1='T',var2='T'):
...
def plot_quadrant_profiles(fig1,axT,axO,AXBTq,Argoq,NGEq,Gliq,Buoyq,Sailq,minrow,maxrow,mincol,maxcol,mdlds=None,var1='T',var2='OHC'):
    ...
    if ( (~np.isnan(xlon)) & ((AXBTq.T[xix].max()-AXBTq.T[xix].min())>0.2) ):
      ...
      if ( NGEq is not None ):
        ...
        if ( len(doubleix) > 0 ):
          ...
          m,mm,l,ml=plot_profile(axT,axO,axP,NGEq.lon[ix],NGEq.lat[ix],-NGEq.pressure,NGEqVar1,OHC=NGEqVar2,mdlds=None,color='orange',marker='o',var1=var1,var2=var2);
          if ( label5 is None ):
            label5 = 'NGE-OHC';
      ...
      if ( len(dists) > 0 ):
        ...
        if ( len(doubleix) > 0 ):
          ...
          m,mm,l,ml=plot_profile(axT,axO,axP,argo.LONGITUDE,argo.LATITUDE,-argo.PRES,argoVar1,OHC=argoVar2,mdlds=mdlds,color='lightgreen',marker='s',linewidth=3.0,var1=var1,var2=var2);
          if ( label3 is None ):
            label3 = 'Argo';
      ...
      m,mm,l,ml=plot_profile(axT,axO,axP,AXBTq.lon[xix],AXBTq.lat[xix],-AXBTq.z,axbtVar1,axbtVar2,mdlds=mdlds,color='k',var1=var1,var2=var2);
      if ( label1 is None ):
        label1 = 'AXBT';
...
def main():
  ...
  for (FILE,fff) in zip(UNPLOTTED_LIST,np.array(range(UNPLOTTED_LIST.size))):
    ...
    tspeedkts = np.double(ATCF_DATA[list(FHRIND),27][0])/10
    tdir = np.double(ATCF_DATA[list(FHRIND),26][0])
    ...
    if do_sst_ohc_profiles == 'Y':
      ...
      plot_quadrant_profiles(fig1,axT,axO,AXBTsNW,ArgosNW,NGEs,None,None,None,4,5,0,2,ds,var1='T',var2='OHC')
      plot_quadrant_profiles(fig1,axT,axO,AXBTsNE,ArgosNE,NGEs,None,None,None,4,5,3,5,ds,var1='T',var2='OHC')
      plot_quadrant_profiles(fig1,axT,axO,AXBTsSW,ArgosSW,NGEs,None,None,None,6,6,0,2,ds,var1='T',var2='OHC')
      plot_quadrant_profiles(fig1,axT,axO,AXBTsSE,ArgosSE,NGEs,None,None,None,6,6,3,5,ds,var1='T',var2='OHC')
    ...
    if do_sst_mlt_profiles == 'Y':
      ...
      plot_quadrant_profiles(fig1,axT,axO,AXBTsNW,ArgosNW,NGEs,None,None,None,4,5,0,2,ds,var1='T',var2='T')
      plot_quadrant_profiles(fig1,axT,axO,AXBTsNE,ArgosNE,NGEs,None,None,None,4,5,3,5,ds,var1='T',var2='T')
      plot_quadrant_profiles(fig1,axT,axO,AXBTsSW,ArgosSW,NGEs,None,None,None,6,6,0,2,ds,var1='T',var2='T')
      plot_quadrant_profiles(fig1,axT,axO,AXBTsSE,ArgosSE,NGEs,None,None,None,6,6,3,5,ds,var1='T',var2='T')
    ...
    if do_iso_26_mlt_profiles == 'Y':
      ...
      plot_quadrant_profiles(fig1,axT,axO,AXBTsNW,ArgosNW,NGEs,None,None,None,4,5,0,2,ds,var1='i26',var2='T')
      plot_quadrant_profiles(fig1,axT,axO,AXBTsNE,ArgosNE,NGEs,None,None,None,4,5,3,5,ds,var1='i26',var2='T')
      plot_quadrant_profiles(fig1,axT,axO,AXBTsSW,ArgosSW,NGEs,None,None,None,6,6,0,2,ds,var1='i26',var2='T')
      plot_quadrant_profiles(fig1,axT,axO,AXBTsSE,ArgosSE,NGEs,None,None,None,6,6,3,5,ds,var1='i26',var2='T')
    ...
    if do_mlt_mld_profiles == 'Y':
      ...
      plot_quadrant_profiles(fig1,axT,axO,AXBTsNW,ArgosNW,NGEs,None,None,None,4,5,0,2,ds,var1='T',var2='D')
      plot_quadrant_profiles(fig1,axT,axO,AXBTsNE,ArgosNE,NGEs,None,None,None,4,5,3,5,ds,var1='T',var2='D')
      plot_quadrant_profiles(fig1,axT,axO,AXBTsSW,ArgosSW,NGEs,None,None,None,6,6,0,2,ds,var1='T',var2='D')
      plot_quadrant_profiles(fig1,axT,axO,AXBTsSE,ArgosSE,NGEs,None,None,None,6,6,3,5,ds,var1='T',var2='D')
    ...
    if do_sss_mls_profiles == 'Y':
      ...
      plot_quadrant_profiles(fig1,axT,axO,None,ArgosNW,NGEs,None,None,None,4,5,0,2,ds,var1='S',var2='S')
      plot_quadrant_profiles(fig1,axT,axO,None,ArgosNE,NGEs,None,None,None,4,5,3,5,ds,var1='S',var2='S')
      plot_quadrant_profiles(fig1,axT,axO,None,ArgosSW,NGEs,None,None,None,6,6,0,2,ds,var1='S',var2='S')
      plot_quadrant_profiles(fig1,axT,axO,None,ArgosSE,NGEs,None,None,None,6,6,3,5,ds,var1='S',var2='S')
