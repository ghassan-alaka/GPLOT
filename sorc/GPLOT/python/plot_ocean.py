...
def main():
        ...
        if ODIR_TYPE == 1:
                ...
                ODIR = ODIR+'/'+EXPT.strip()+'/'+IDATE.strip()+'/ocean/'
        ...
        UNPLOTTED_FILE = ODIR.strip()+'UnplottedOceanFiles.'+DOMAIN.strip()+'.'+TIER.strip()+'.'+SID.strip()+'.log'
        PLOTTED_FILE = ODIR.strip()+'PlottedOceanFiles.'+DOMAIN.strip()+'.'+TIER.strip()+'.'+SID.strip()+'.log'
        ...
        for (FILE,fff) in zip(UNPLOTTED_LIST,np.array(range(UNPLOTTED_LIST.size))):
                ...
                if (figuretest < 1):
                        ...
                        if (gribfiletest < 1):
                                ...
                                # ds = all_ds.where((lonmin<=all_ds.Longitude) & (all_ds.Longitude<=lonmax) & (latmin<=all_ds.Latitude) & (all_ds.Latitude<=latmax));
                                ds = all_ds
                                ...
                                MLD.plot(figsize=(11,9)); plt.savefig('3z.png');
                                ...
                                if (ofiletest < 1):
                                        ...
                                        # ods = all_ods.where((lonmin<=all_ods.Longitude) & (all_ods.Longitude<=lonmax) & (latmin<=all_ods.Latitude) & (all_ods.Latitude<=latmax));
                                        ods = all_ods
                                        SHF = ods.surface_heat_flux.squeeze()
                                        SSH = ods.sea_surface_height.squeeze()
                                        print(('SSH',SSH.max().values))
                                ...
                                if os.path.exists(f'{NMLDIR}/namelist.ocean.structure.{EXPT}'):
                                        namelist_structure_vars = np.genfromtxt(f'{NMLDIR}/namelist.ocean.{EXPT}',delimiter=',',dtype='str')
                                else:
                                        namelist_structure_vars = np.genfromtxt(f'{NMLDIR}/namelist.ocean',delimiter=',',dtype='str')
                                ...
                                do_ssh = namelist_structure_vars[5,1]
                                do_dpi = namelist_structure_vars[6,1]
                                ...
                                sfc_conv_levs = np.arange(-300,300+1e-6,50.0);  sfc_conv_ticks = np.arange(-300,300+1e-6,50.0)
                                sfc_vort_levs = np.arange(-300,300+1e-6,50.0);  sfc_vort_ticks = np.arange(-300,300+1e-6,50.0)
                                SSH_levs = np.arange(-200,200+1e-6,20.0);       SSH_ticks = np.arange(-200,200+1e-6,20.0)
                                DPI_levs = np.arange(0,80+1e-6,5.0);            DPI_ticks = np.arange(0,80+1e-6,5.0)
                                
                                # u_x = ucurr.differentiate('Longitude');
                                # u_y = ucurr.differentiate('Latitude');
                                # v_x = vcurr.differentiate('Longitude');
                                # v_y = vcurr.differentiate('Latitude');
                                lonstretch = np.cos(np.deg2rad(lon.mean()));
                                u_x = MLu.differentiate('Longitude') / 1e2 / (lonstretch*111e3) # [cm/s/degree] => [m/s]
                                u_y = MLu.differentiate('Latitude') / 1e2 / 111e3
                                v_x = MLv.differentiate('Longitude') / 1e2 / (lonstretch*111e3)
                                v_y = MLv.differentiate('Latitude') / 1e2 / 111e3
                                sfc_conv = -(u_x + v_y);                # Divergence on model layer
                                sfc_vort = (v_x - u_y);                 # Relative vorticity on model layer
                                
                                # Dynamic potential intensity (Balaguru et al. 2015)
                                rho0 = 1000.0;  # Reference density
                                ustar = 0.3;	# [m/s] Frictional velocity HACK HACK HACK
                                U = 10; #[m/s]
                                ...
                                hg = 2549       # []
                                # HACK HACK HACK
                                aT = 26         # [oC] Air temperature atmospheric boundary layer HACK HACK HACK
                                ...
                                if do_iso_26 == 'Y':
                                        ...
                                        figfname = ODIR+'/'+LONGSID.lower()+'.iso_26.'+forecastinit+'.ocean.f'+format(FHR,'03d')
                                ...
                                if do_iso_20 == 'Y':
                                        ...
                                        ax1.set_title('VMAX= '+maxwind+' kt'+'\n'+'PMIN= '+minpressure+' hPa'+'\n'+LONGSID.upper(),fontsize=fontsize,color='brown',loc='right') #fontsize=24
                                        figfname = ODIR+'/'+LONGSID.lower()+'.iso_20.'+forecastinit+'.ocean.f'+format(FHR,'03d')
                                        fig1.savefig(figfname+figext, bbox_inches='tight', dpi='figure')
                                        if ( DO_CONVERTGIF ):
                                                os.system(f"convert {figfname}{figext} +repage gif:{figfname}.gif && /bin/rm {figfname}{figext}")
                                        plt.close(fig1)
                                ...
                                if do_ohc == 'Y':
                                        ...
                                        cbar1.ax.tick_params(labelsize=fontsize) #labelsize=24
                                        Axes.streamplot(ax1,xi,yi,MLu,MLv,color='gray',density=0.5);
                                        ax1.set_title(EXPT.strip()+'\n'+ r'Ocean Heat Content ($J\ cm^2$, Shading), U$_{MLD}$ ($cm\ s^{-1}$, Strmlns.)'+'\n'+'Init: '+forecastinit+' Forecast Hour:[{:03d}]'.format(FHR),fontsize=small_fontsize, weight = 'bold',loc='left') #fontsize=24
                                        ax1.set_title('VMAX= '+maxwind+' kt'+'\n'+'PMIN= '+minpressure+' hPa'+'\n'+LONGSID.upper(),fontsize=fontsize,color='brown',loc='right') #fontsize=24
                                        figfname = ODIR+'/'+LONGSID.lower()+'.ohc.'+forecastinit+'.ocean.f'+format(FHR,'03d')
                                        fig1.savefig(figfname+figext, bbox_inches='tight', dpi='figure')
                                        if ( DO_CONVERTGIF ):
                                                os.system(f"convert {figfname}{figext} +repage gif:{figfname}.gif && /bin/rm {figfname}{figext}")
                                        plt.close(fig1)
                                ...
                                if do_sfc_conv == 'Y':
                                        fig1 = plt.figure(figsize=figsize)
                                        ax1 = fig1.add_subplot(1, 1, 1)
                                        # co1 = ax1.contourf(lon,lat, sfc_conv, extend='both')
                                        # plt.colorbar(co1)
                                        ...
                                        ax1.set_title(EXPT.strip()+'\n'+ r'Ocean Heat Content ($J\ cm^2$, Shading), U$_{MLD}$ ($cm\ s^{-1}$, Strmlns.)'+'\n'+'Init: '+forecastinit+' Forecast Hour:[{:03d}]'.format(FHR),fontsize=small_fontsize, weight = 'bold',loc='left') #fontsize=24
                                        ax1.set_title('VMAX= '+maxwind+' kt'+'\n'+'PMIN= '+minpressure+' hPa'+'\n'+LONGSID.upper(),fontsize=fontsize,color='brown',loc='right') #fontsize=24
                                        figfname = ODIR+'/'+LONGSID.lower()+'.sfc_conv.'+forecastinit+'.ocean.f'+format(FHR,'03d')
                                        fig1.savefig(figfname+figext, bbox_inches='tight', dpi='figure')
                                        if ( DO_CONVERTGIF ):
                                                os.system(f"convert {figfname}{figext} +repage gif:{figfname}.gif && /bin/rm {figfname}{figext}")
                                        plt.close(fig1)
                                ...
                                if do_sfc_vort == 'Y':
                                        fig1 = plt.figure(figsize=figsize)
                                        ax1 = fig1.add_subplot(1, 1, 1)
                                        # co1 = ax1.contourf(lon,lat, sfc_vort, extend='both')
                                        # plt.colorbar(co1)
                                        ...
                                        ax1.set_title(EXPT.strip()+'\n'+ r'Ocean Heat Content ($J\ cm^2$, Shading), U$_{MLD}$ ($cm\ s^{-1}$, Strmlns.)'+'\n'+'Init: '+forecastinit+' Forecast Hour:[{:03d}]'.format(FHR),fontsize=small_fontsize, weight = 'bold',loc='left') #fontsize=24
                                        ax1.set_title('VMAX= '+maxwind+' kt'+'\n'+'PMIN= '+minpressure+' hPa'+'\n'+LONGSID.upper(),fontsize=fontsize,color='brown',loc='right') #fontsize=24
                                        figfname = ODIR+'/'+LONGSID.lower()+'.sfc_vort.'+forecastinit+'.ocean.f'+format(FHR,'03d')
                                        fig1.savefig(figfname+figext, bbox_inches='tight', dpi='figure')
                                        if ( DO_CONVERTGIF ):
                                                os.system(f"convert {figfname}{figext} +repage gif:{figfname}.gif && /bin/rm {figfname}{figext}")
                                        plt.close(fig1)
                                # FIGURE: Sea-surface height [m]
                                if do_ssh == 'Y' and SSH is not None:
                                        ...
                                        cbar1.ax.tick_params(labelsize=fontsize) #labelsize=24
                                        Axes.streamplot(ax1,xi,yi,MLu,MLv,color='gray',density=0.5);
                                        ax1.set_title(EXPT.strip()+'\n'+ r'Dynamic Potential Intensity ($J\ cm^2$, Shading), U$_{MLD}$ ($cm\ s^{-1}$, Strmlns.)'+'\n'+'Init: '+forecastinit+' Forecast Hour:[{:03d}]'.format(FHR),fontsize=small_fontsize, weight = 'bold',loc='left') #fontsize=24
                                        ax1.set_title('VMAX= '+maxwind+' kt'+'\n'+'PMIN= '+minpressure+' hPa'+'\n'+LONGSID.upper(),fontsize=fontsize,color='brown',loc='right') #fontsize=24
                                        figfname = ODIR+'/'+LONGSID.lower()+'.ssh.'+forecastinit+'.ocean.f'+format(FHR,'03d')
                                        fig1.savefig(figfname+figext, bbox_inches='tight', dpi='figure')
                                        if ( DO_CONVERTGIF ):
                                                os.system(f"convert {figfname}{figext} +repage gif:{figfname}.gif && /bin/rm {figfname}{figext}")
                                        plt.close(fig1)
                                ...
                                if do_dpi == 'Y':
                                        ...
                                        ax1.set_title(EXPT.strip()+'\n'+ r'Dynamic Potential Intensity ($J\ cm^2$, Shading), U$_{MLD}$ ($cm\ s^{-1}$, Strmlns.)'+'\n'+'Init: '+forecastinit+' Forecast Hour:[{:03d}]'.format(FHR),fontsize=small_fontsize, weight = 'bold',loc='left') #fontsize=24
                                        ax1.set_title('VMAX= '+maxwind+' kt'+'\n'+'PMIN= '+minpressure+' hPa'+'\n'+LONGSID.upper(),fontsize=fontsize,color='brown',loc='right') #fontsize=24
                                        figfname = ODIR+'/'+LONGSID.lower()+'.dpi.'+forecastinit+'.ocean.f'+format(FHR,'03d')
                                        fig1.savefig(figfname+figext, bbox_inches='tight', dpi='figure')
                                        if ( DO_CONVERTGIF ):
                                                os.system(f"convert {figfname}{figext} +repage gif:{figfname}.gif && /bin/rm {figfname}{figext}")
                                        plt.close(fig1)
