...
_CMAP_REGISTRY = {
    ...
    # Liutex (rigid-rotation strength, 10^-5 s^-1). Total magnitude is a
    # non-negative scalar -> reuse the RVO sequential 'hot_r' scale.
...
_LEVEL_REGISTRY = {
    ...
    # Liutex (10^-5 s^-1). Total magnitude uses the same 5.. floor as RVO so
    # ambient noise falls into set_under (white). The vertical component is a
    # compact, coherent cyclonic core (strong in the low-mid troposphere,
    # weakening aloft) on a signed RdBu_r scale about zero. Level-less
    # fallbacks cover all four pressure levels (850/700/500/200).
    ('LIUTEX', ''): np.arange(5, 155, 10),
