* Skew-T script for HEDAS Model

* Header script by Altug Aksoy and Bryan Williams
* Skew-T function by Bob Hart


function center_range(args)

t_hour = subwrd(args,1)
name = subwrd(args,2)
date = subwrd(args,3)
ctlfile = subwrd(args,4)
center_lon = subwrd(args,5)
center_lat = subwrd(args,6)
...
say "*******************************************************************************************"
say "*  Model forecast hour: "t_hour 
say "*******************************************************************************************"
...
say "grid size "numx" x  "numy
halfx=numx - 1
halfx=halfx/2.
halfy=numy - 1
halfy=halfy/2.
say "grid center "halfx" "halfy

**Find storm center x/y
    distmin=1e20
    ii = 1
    while (ii <= numx)
        "set x "ii
        ...
        if (lon >= -180) & (lon <= 180)
            distlon=math_abs(lon-center_lon)
            if distlon < distmin
                distmin = distlon
                imin = ii
            endif
    ...
    while (jj <= numy)
        ...
        lat=subwrd(result,4)
        if (lat >= -90) & (lat <= 90)
            distlat=math_abs(lat-center_lat)
            if distlat < distmin
                distmin = distlat
                jmin = jj
            endif
        ...
        jj = jj + 1
    endwhile
    say "Storm center i and j found as "imin" and "jmin

**Find first latitude/longitude
    start_lat = center_lat - jmin*0.02
    start_lon = center_lon - imin*0.02
    end_lat   = start_lat + 8
    end_lon   = start_lon + 10

**Set the radius (R) to specified distance.
R = 200

**This function will mask every other value outside the R-km radius.
circ_mask(center_lat,center_lon,R)

**Set up the environment and constants.
"set lat "center_lat
"set lon "center_lon
"set lev 1000 100"
_pi=3.14159265
...
**This converts lat/lon points to GrADS grid points (x,y).
    distance_y=math_int(((R/111))/0.02)
    distance_x=math_int((((R/111)*cos(_pi/center_lat)))/0.02)
xcent=imin
ycent=jmin


say 'Drawing Skew-T for a 'R' km radius around the center...'

** Debugging

** The following sets the bounds for averaging.

** Set xmin
xmin=xcent-distance_x
if xmin < 1
     xmin = 1
endif

** Set xmax.
xmax=xcent+distance_x
if xmax > numx
     xmax = numx
endif

** Set ymin.
ymin=ycent-distance_y
if ymin < 1
     ymin = 1
endif

** Set ymax.
ymax=ycent+distance_y
if ymax > numy
     ymax = numy
endif

** Define values for Skew-T. (Temp, Td, Wind Speed, Wind Direction)
"define tc=aave(TMPprs-273.15, x="xmin", x="xmax", y="ymin", y="ymax")"
"define dew=aave(DPTprs-273.15, x="xmin", x="xmax", y="ymin", y="ymax")"
"define uavg=aave(UGRDprs, x="xmin", x="xmax", y="ymin", y="ymax")"
"define vavg=aave(VGRDprs, x="xmin", x="xmax", y="ymin", y="ymax")"
"define Spd=mag(uavg*1.9422,vavg*1.9422)"
"define Dir=57.3*atan2(uavg,vavg)+180"
*"define Dir=270-"_rtd"*aave(atan2(-uu,-vv), x="xmin", x="xmax", y="ymin", y="ymax")"
say "u v dir "uavg" "vavg" "Dir
**Draw the Skew-T, and output to file.
rc=plotskew(tc,dew,Spd,Dir)
...
If (PageType = "Portrait")
   "set string 2 c"
   "draw string 0.9 10.1 Experimental Product"
   "draw title Average within "R" km of the center\"name" "date" "t_hour"h \Center LON= "center_lon", LAT= "center_lat
Else
   "set string 2 c"
   "draw string 0.9 8.4 Experimental Product"
   "draw title Average within "R" km of the center\"name" "date" "t_hour"h \Center LON= "center_lon", LAT= "center_lat
Endif

"printim "name"."date"."t_hour"hr.SKEWT_CENTER200KM.gif gif"
*"printim skew_center_200km_"t_hour"hr.gif gif"
"enable print skew"xlatll""xlonll".gm"
"print"
"disable print"

**Skew-T for NE Quadrant:

say 'Drawing Skew-T for NE quadrant of the 'R' km radius around the center...'
...
"define vavg=aave(VGRDprs, x="xmin", x="xmax", y="ymin", y="ymax")"
"define Spd=mag(uavg*1.9422,vavg*1.9422)"
*"define Dir=270-"_rtd"*atan2(-uavg,-vavg)"
"define Dir=57.3*atan2(uavg,vavg)+180"
*"define Dir=270-"_rtd"*aave(atan2(-uu,-vv), x="xmin", x="xmax", y="ymin", y="ymax")"
say "u v dir "uavg" "vavg" "Dir
"clear"
...
If (PageType = "Portrait")
   "set string 2 c"
   "draw string 0.9 10.1 Experimental Product"
   "draw title Average in NE quadrant within "R" km of the center\"name" "date" "t_hour"h \Center LON= "center_lon", LAT= "center_lat
Else
   "set string 2 c"
   "draw string 0.9 8.4 Experimental Product"
   "draw title Average in NE quadrant within "R" km of the center\"name" "date" "t_hour"h \Center LON= "center_lon", LAT= "center_lat
...
"printim "name"."date"."t_hour"hr.SKEWT_NE.gif gif"
*"printim skew_NE_"t_hour"hr.gif gif"
"enable print skew"xlatll""xlonll".gm"
"print"
"disable print"

**Skew-T for NW Quadrant:

say 'Drawing Skew-T for NW quadrant of the 'R' km radius around the center...'
...
If (PageType = "Portrait")
   "set string 2 c"
   "draw string 0.9 10.1 Experimental Product"
   "draw title Average in NW quadrant within "R" km of the center\"name" "date" "t_hour"h \Center LON= "center_lon", LAT= "center_lat
Else
   "set string 2 c"
   "draw string 0.9 8.4 Experimental Product"
   "draw title Average in NW quadrant within "R" km of the center\"name" "date" "t_hour"h \Center LON= "center_lon", LAT= "center_lat
...
"printim "name"."date"."t_hour"hr.SKEWT_NW.gif gif"
*"printim skew_NW_"t_hour"hr.gif gif"
"enable print skew"xlatll""xlonll".gm"
"print"
"disable print"

**Skew-T for SW Quadrant:

say 'Drawing Skew-T for SW quadrant of the 'R' km radius around the center...'
...
If (PageType = "Portrait")
   "set string 2 c"
   "draw string 0.9 10.1 Experimental Product"
   "draw title Average in SW quadrant within "R" km of the center\"name" "date" "t_hour"h \Center LON= "center_lon", LAT= "center_lat
Else
   "set string 2 c"
   "draw string 0.9 8.4 Experimental Product"
   "draw title Average in SW quadrant within "R" km of the center\"name" "date" "t_hour"h \Center LON= "center_lon", LAT= "center_lat
...
"printim "name"."date"."t_hour"hr.SKEWT_SW.gif gif"
*"printim skew_SW_"t_hour"hr.gif gif"
"enable print skew"xlatll""xlonll".gm"
"print"
"disable print"

**Skew-T for SE Quadrant:

say 'Drawing Skew-T for SE quadrant of the 'R' km radius around the center...'
...
If (PageType = "Portrait")
   "set string 2 c"
   "draw string 0.9 10.1 Experimental Product"
   "draw title Average in SE quadrant within "R" km of the center\"name" "date" "t_hour"h \Center LON= "center_lon", LAT= "center_lat
Else
   "set string 2 c"
   "draw string 0.9 8.4 Experimental Product"
   "draw title Average in SE quadrant within "R" km of the center\"name" "date" "t_hour"h \Center LON= "center_lon", LAT= "center_lat
...
"printim "name"."date"."t_hour"hr.SKEWT_SE.gif gif"
*"printim skew_SE_"t_hour"hr.gif gif"
"enable print skew"xlatll""xlonll".gm"
"print"
"disable print"
...
say "*************************************************"
say "End of the time loop reached. All charts plotted!"
say "*************************************************"
...
If (DrawPLev = 1 & LblAxes = 1)
   ...
   "set string 1 c 3 90"
   If (PageType = "Portrait") 
      "draw string 0.15 4.75 Pressure (mb)"
   Else
      "draw string 0.15 4.25 Pressure(mb)"
   Endif
Endif
...
function circ_mask(lat0,lon0,R)
...
*   Cartesian coordiantes on unit sphere
    ...
    'x0 =  cos(d2r*'lat0') * cos(d2r*'lon0')'
    'y0 =  cos(d2r*'lat0') * sin(d2r*'lon0')'
    'z0 =  sin(d2r*'lat0')'

*   Great circle distance o (lon0,lat0)

    'dist = a * acos(x*x0 + y*y0 + z*z0)'

*   Mask

    'mask = maskout(1+0*lat,R-dist)'
